...
func fetchGoods() {
 ...
 AF.request("http://allmarket.armenianbros.com/api/v2/products?city_id=6")
 .validate()
 .responseJSON { (data) in
      print(data)
    }
 .responseDecodable(of: GoodsModel.self) { (response) in
   guard let goods = response.value else { return }
  self.goodData = goods.allGoods
  self.tableView.reloadData()
  print(goods.allGoods[0].title)
